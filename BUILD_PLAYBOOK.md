...
- **Local-environment circuit-breaker for long-running crawlers.** Any polite long-running crawler that records faults *as data* needs a tripwire that distinguishes "a fault about the target" from "a fault about my own environment" — otherwise an environmental failure (DNS outage, lost connectivity, a sleep gap) is silently recorded as a wall of target failures and can reach the verdict as a false finding. A run that exits zero with output that "looks complete" is the most dangerous shape of this bug. Recommended form: if N consecutive faults of the same network class (esp. `dns_failure`) occur across multiple *distinct* hosts within M seconds, pause and surface an alert rather than record-and-continue; treat the baseline fault rate of earlier clean work as the implausibility threshold. The worked example: the IATI access probe's D7 crawl hit a local DNS outage at country 10 of 18 — zero `dns_failure` across the first nine countries (0/3,600), then 354/400 in country ten and 400/400 across eight more, simultaneously with git pushes failing to resolve the host. Caught in review before the verdict; the affected country was surgically cleaned and the rest re-crawled. (Origin: IATI Corpus Access Probe, Sprint 1. Recommendation for *future* crawl-based probes — not retrofitted into this one.)
